...
class Spells:
    def __init__(self, game):
        self.game = game
        self.spells = []
        self.name_spells = ["smallfire", "smallthunder", "boulder", "magicshield", "selfheal",
                            "bard", "freeze", "blank", "fireball", "frostbullet"]
        self.description = ["Opis zaklęcia smallfire",
    ...
    def draw(self):
        for i, spell in enumerate(self.sprites_list[:4]):                   # iterowanie po skillach, które można wybrać
            if i != self.change_spell or not self.spell_moving:             # nie rysujemy przenoszonego spella
                if i == self.chosen:
                    ...
                    self.game.screen.blit(tmp, (7 * 50 + i * (self.size + 3) - 2.5, 5 - 2.5))
                self.game.screen.blit(spell.image, spell.rect)
                # wyświetlanie braku możliwości wybrania skilla
                self.draw_blocking_time(spell, i)
        ...
        if self.flag_ctrl and self.special != 7:                            # podświetlenie specjalnego slotu
            ...
            tmp.fill(GREEN, special_flags=pygame.BLEND_MAX)
            self.game.screen.blit(tmp, (11 * 50 - 2.5, 5 - 2.5))
        self.game.screen.blit(self.special_slot[self.special - 7].image, (11 * 50, 5))      # specjalny slot
        self.draw_description()

    def draw_blocking_time(self, spell, i):
        ...
        if now_time - last_cast <= block_time:
            ...
            self.game.screen.blit(tmp, spell.rect)
            self.game.screen.blit(text, (spell.rect.x + 15 - len(str(time_to_end) * 3), spell.rect.y + 10))
    ...
    def draw_description(self):
        for i, spell in enumerate(self.sprites_list):
            if spell.rect.collidepoint(pygame.mouse.get_pos()):
                ...
                self.game.screen.blit(desc, (x + 15, y + 15))
    ...
    def draw_moving_item(self):
        if self.spell_moving:
            self.game.screen.blit(self.spell, (self.spell_x, self.spell_y))
    ...
    def change_positions(self, position):
        ...
        if self.change_spell is None:
            ...
            self.sprites_list[self.change_spell], self.sprites_list[position] = \
                self.sprites_list[position], self.sprites_list[self.change_spell]
